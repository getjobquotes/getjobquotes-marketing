#!/bin/bash
# ============================================================
# GetJobQuotes — Full Feature Install (no Stripe)
...
echo ""
echo "╔════════════════════════════════════════╗"
echo "║   GetJobQuotes — Full Feature Install  ║"
echo "╚════════════════════════════════════════╝"
echo ""
...
# ============================================================
# 2. AUTH PAGE — magic link + password login + returning user
# ============================================================
echo "🔐 Building auth page..."
mkdir -p app/auth
...
function AuthInner() {
  ...
  const [mode, setMode] = useState<"magic" | "password" | "signup">(
    defaultMode === "signup" ? "signup" : "magic"
  );
  ...
  useEffect(() => {
    const name = localStorage.getItem("gjq_display_name");
    const savedEmail = localStorage.getItem("gjq_email");
    if (name) setReturningUser(name);
