    ...
    <div className="min-h-screen bg-[rgb(var(--bg))] text-[rgb(var(--text))] flex flex-col">
      ...
      <div className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-[rgb(var(--text-muted))] text-sm mt-1">Here's your overview.</p>
        </div>
        ...
        {showUpgradedBanner && (
          <div className="mb-4 rounded-xl border border-green-500/30 bg-green-500/10 px-4 py-3 flex items-center justify-between">
            ...
            <button onClick={() => setShowUpgradedBanner(false)} className="text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text))] text-lg">×</button>
          </div>
        )}
        ...
        {/* Plan bar */}
        {!plan.loading && (
          <div className="mb-4 rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--surface)/0.5)] px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              {plan.isPro ? (
                <>
                  ...
                  <span className="text-xs text-[rgb(var(--text-muted))]">Unlimited quotes</span>
                </>
              ) : (
                <>
                  <span className="text-xs px-2 py-0.5 rounded-full bg-[rgb(var(--surface2))] text-[rgb(var(--text-muted))] border border-[rgb(var(--border-strong))] font-semibold">FREE</span>
                  <span className="text-xs text-[rgb(var(--text-muted))]">{plan.quotesThisMonth}/5 quotes this month</span>
                </>
              )}
            </div>
            {!plan.isPro && (
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {[
            ...
            <div key={s.label} className={`rounded-2xl border p-4 ${(s as any).alert ? "border-red-500/30 bg-red-500/5" : "border-[rgb(var(--border))] bg-[rgb(var(--surface)/0.5)]"}`}>
              <p className="text-xs text-[rgb(var(--text-muted))] uppercase tracking-widest mb-2">{s.label}</p>
              <p className="text-2xl font-bold">{s.value}</p>
              {(s as any).sub && <p className={`text-xs mt-1 ${(s as any).alert ? "text-red-400" : "text-[rgb(var(--text-faint))]"}`}>{(s as any).sub}</p>}
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          ...
          <Link href="/profile" className="px-4 py-2.5 text-sm border border-[rgb(var(--border))] hover:border-[rgb(var(--border-strong))] rounded-xl text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text))] transition">🏢 Profile</Link>
          <Link href="/customers" className="px-4 py-2.5 text-sm border border-[rgb(var(--border))] hover:border-[rgb(var(--border-strong))] rounded-xl text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text))] transition">👥 Customers</Link>
          <Link href="/help" className="px-4 py-2.5 text-sm border border-[rgb(var(--border))] hover:border-[rgb(var(--border-strong))] rounded-xl text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text))] transition">❓ Help</Link>
        </div>
        ...
        {!docsLoading && docs.length === 0 && !error && (
          <div className="rounded-2xl border border-dashed border-[rgb(var(--border))] p-12 text-center">
            ...
            <p className="text-[rgb(var(--text-muted))] text-sm mb-6">Create your first professional quote in under 2 minutes.</p>
        ...
        {!docsLoading && docs.length > 0 && (
          <>
              ...
              <div className="flex gap-1 p-1 bg-[rgb(var(--surface))] border border-[rgb(var(--border))] rounded-full">
                {(["quotes", "invoices"] as const).map(t => (
                  <button key={t} onClick={() => { setTab(t); setSearch(""); setSelected(new Set()); }}
                    className={`px-4 py-1.5 rounded-full text-sm font-semibold capitalize transition ${tab === t ? "bg-green-600 text-[rgb(var(--text))]" : "text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text))]"}`}>
            ...
            <div className="relative mb-4">
              <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[rgb(var(--text-muted))]">🔍</span>
              <input
                value={search}
                onChange={e => { setSearch(e.target.value); setSelected(new Set()); }}
                placeholder={`Search ${tab} by client name or number...`}
                className="w-full pl-10 pr-10 py-3 rounded-xl border border-[rgb(var(--border-strong))] bg-[rgb(var(--surface))] text-[rgb(var(--text))] text-sm placeholder:text-[rgb(var(--text-faint))] outline-none focus:border-green-500 transition"
              />
              {search && (
                <button onClick={() => setSearch("")}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text))] text-lg">×</button>
              )}
            </div>
            ...
            {selected.size > 0 && (
              <div className="mb-3 rounded-xl border border-red-900/30 bg-red-500/5 px-4 py-2.5 flex items-center justify-between">
                <span className="text-sm text-[rgb(var(--text))]">{selected.size} selected</span>
                <div className="flex gap-2">
                  <button onClick={() => setSelected(new Set())}
                    className="text-xs px-3 py-1.5 rounded-lg border border-[rgb(var(--border-strong))] text-[rgb(var(--text-muted))] hover:text-[rgb(var(--text))] transition">
                    Clear
                  </button>
                  <button onClick={handleBulkDelete} disabled={bulkDeleting}
                    className="text-xs px-3 py-1.5 rounded-lg bg-red-600 hover:bg-red-500 text-[rgb(var(--text))] transition disabled:opacity-50">
            ...
            {!search && recent.length > 0 && (
              <div className="mb-4">
                <p className="text-xs text-[rgb(var(--text-faint))] uppercase tracking-widest mb-2 px-1">Recent</p>
                <div className="flex gap-2 overflow-x-auto pb-1">
                  {recent.map(doc => (
                    <Link key={doc.id} href={`/tool?id=${doc.id}`}
                      className="shrink-0 rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--surface)/0.5)] px-3 py-2 hover:border-[rgb(var(--border-strong))] transition min-w-[140px]">
                      <p className="text-xs font-semibold text-[rgb(var(--text))] truncate">{doc.client_name || "—"}</p>
                      <p className="text-xs text-[rgb(var(--text-faint))] mt-0.5">{fmt(doc.total)}</p>
                    </Link>
                  ))}
                </div>
              </div>
            ...
            <div className="space-y-2">
              {filtered.length === 0 ? (
                <p className="text-center text-[rgb(var(--text-faint))] text-sm py-8">
                ...
                return (
                  <div key={doc.id}
                    className={`rounded-2xl border px-4 py-3.5 transition ${
                      ...
                      "border-[rgb(var(--border))] bg-[rgb(var(--surface)/0.4)] hover:border-[rgb(var(--border-strong))]"
                    }`}>
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <button
                        ...
                        className={`mt-0.5 w-4 h-4 rounded border-2 shrink-0 flex items-center justify-center transition ${
                          isSelected ? "bg-green-600 border-green-600" : "border-[rgb(var(--border-strong))] hover:border-green-500"
                        }`}>
                        {isSelected && <span className="text-[rgb(var(--text))] text-[10px]">✓</span>}
                      </button>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-[rgb(var(--text-faint))] mt-0.5">
                    ...
                    <div className="flex gap-2 mt-3 flex-wrap pl-7">
                      <Link href={`/tool?id=${doc.id}`}
                        className="text-xs px-3 py-1.5 rounded-lg border border-[rgb(var(--border-strong))] hover:border-[rgb(var(--border-strong))] text-[rgb(var(--text))] hover:text-[rgb(var(--text))] transition">
                        Edit
                      </Link>
                      {doc.type === "quote" && (
                        <button onClick={() => handleConvert(doc)}
                          className="text-xs px-3 py-1.5 rounded-lg border border-[rgb(var(--border-strong))] hover:border-green-500/50 text-[rgb(var(--text))] hover:text-green-400 transition">
                          → Invoice
                        </button>
                      )}
                      {doc.type === "invoice" && doc.status !== "paid" && (
                        <button onClick={() => handlePaid(doc)} disabled={markingPaid === doc.id}
                          className="text-xs px-3 py-1.5 rounded-lg border border-green-700/40 hover:bg-green-600 text-green-400 hover:text-[rgb(var(--text))] transition disabled:opacity-40">
                          {markingPaid === doc.id ? "…" : "✓ Mark Paid"}
                        </button>
                      )}
                      <button onClick={() => handleDelete(doc.id)}
                        className="text-xs px-2 py-1.5 text-[rgb(var(--text-faint))] hover:text-red-400 transition ml-auto">✕</button>
                    </div>
                  </div>
                );
              })}
