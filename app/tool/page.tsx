...
function buildPDF(opts: {
  form: any; lineItems: LineItem[]; subtotal: number;
  vatAmount: number; total: number; sigData: string;
  profile: any; docId: string | null;
}): jsPDF {
      ...
      supabase.from("documents").select("*").eq("id", editId).single()
        .then(({ data: doc }) => {
          ...
          setForm({
            ...
            clientPhone: "", description: doc.description || "",
            vat: doc.vat || false, notes: doc.notes || "",
            expiryDays: "30", type: doc.type || "quote",
          });
