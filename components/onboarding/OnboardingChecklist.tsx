...
export default function OnboardingChecklist({ userId }: { userId: string }) {
  ...
  return (
    <div className="rounded-2xl border border-[rgb(var(--border))] bg-[rgb(var(--surface)/0.4)] overflow-hidden mb-5">
      {/* Header */}
      <div className="px-4 pt-4 pb-3 border-b border-zinc-800/60">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-semibold text-[rgb(var(--text))]">Get set up</h2>
            <span className="text-xs px-2 py-0.5 rounded-full bg-[rgb(var(--surface2))] text-[rgb(var(--text-muted))] tabular-nums">
              {completedCount}/{STEPS.length}
            </span>
          </div>
          <button onClick={dismiss} className="text-[rgb(var(--text-faint))] hover:text-[rgb(var(--text-muted))] text-base transition" title="Dismiss">×</button>
        </div>
        {/* Progress bar */}
        <div className="w-full h-1 bg-[rgb(var(--surface2))] rounded-full overflow-hidden">
      ...
      <div className="divide-y divide-zinc-800/40">
        {STEPS.map(step => {
          ...
          return (
            <div key={step.key}
              className={`flex items-center gap-3 px-4 py-3 transition ${done ? "opacity-50" : isNext ? "bg-[rgb(var(--surface2)/0.2)]" : ""}`}>
              {/* Tick */}
              <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition ${
                done ? "bg-green-600 border-green-600" : isNext ? "border-green-500" : "border-[rgb(var(--border-strong))]"
              }`}>
                {done && (
                  <svg className="w-2.5 h-2.5 text-[rgb(var(--text))]" fill="none" viewBox="0 0 10 10">
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-sm leading-snug ${done ? "line-through text-[rgb(var(--text-faint))]" : "text-zinc-200 font-medium"}`}>
                  {step.label}
                </p>
                {!done && isNext && (
                  <p className="text-xs text-[rgb(var(--text-faint))] mt-0.5 leading-relaxed">{step.desc}</p>
                )}
              </div>
